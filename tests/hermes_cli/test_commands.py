...
class TestGatewayKnownCommands:
    ...
    def test_browser_in_gateway_known_commands(self):
        assert "browser" in GATEWAY_KNOWN_COMMANDS
...
class TestGatewayHelpLines:
    ...
    def test_browser_appears_in_gateway_help(self):
        lines = gateway_help_lines()
        joined = "\n".join(lines)
        assert "`/browser [connect|disconnect|status]`" in joined
