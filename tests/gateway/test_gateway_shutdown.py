...
@pytest.mark.asyncio
async def test_codex_config_watcher_requests_service_restart(tmp_path, monkeypatch):
    runner, adapter = make_restart_runner()
    adapter.disconnect = AsyncMock()
    ...
    monkeypatch.setenv("HERMES_CODEX_BOOTSTRAPPED", "1")
    monkeypatch.setenv("CODEX_HOME", str(codex_home))
    monkeypatch.setenv("HERMES_CODEX_WATCH_INTERVAL", "1")

    with patch("gateway.status.remove_pid_file"), patch("gateway.status.write_runtime_status"):
        runner._start_codex_config_watcher()
        await asyncio.sleep(0.05)
    ...
    assert runner._shutdown_event.is_set() is True
    assert runner._exit_code == GATEWAY_SERVICE_RESTART_EXIT_CODE
    assert runner._exit_reason == "Codex config changed"
