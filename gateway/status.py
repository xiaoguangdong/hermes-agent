...
def remove_pid_file(*, force: bool = False) -> None:
    ...
    Pass ``force=True`` only after separately proving the recorded process is
    stale.  This is needed because stale PID records are usually owned by a
    different process ID than the cleaner.
    """
    try:
        path = _get_pid_path()
