...
def probe_cdp_endpoint(cdp_url: str, timeout: int = 3) -> Dict[str, Any]:
    """Probe a user-supplied CDP endpoint and return structured status details."""
    raw = (cdp_url or "").strip()
    if not raw:
        return {
            "ok": False,
    ...
    try:
        if lowered.startswith(("http://", "https://", "ws://", "wss://")):
            ...
            if lowered.startswith(("http://", "https://")):
                version_url = raw if lowered.endswith("/json/version") else raw.rstrip("/") + "/json/version"
            elif "/devtools/browser/" not in lowered:
                scheme = "http" if lowered.startswith("ws://") else "https"
                netloc = parsed.netloc or parsed.path
                version_url = f"{scheme}://{netloc.rstrip('/')}/json/version"
        else:
        ...
        return {
            ...
            "resolved_url": "",
            "message": f"Invalid CDP endpoint: {exc}",
        }
    ...
    try:
        ...
        return {
            "ok": False,
            "status": "discovery_missing_ws",
            "host": host,
            "port": port,
            "version_url": version_url,
            "resolved_url": "",
            "message": "CDP discovery responded but did not include webSocketDebuggerUrl",
        }
    except Exception as exc:
        try:
            ...
            return {
                ...
                "version_url": version_url,
                "resolved_url": "",
                "message": f"TCP port is reachable but CDP discovery failed: {exc}",
            }
        except Exception:
            return {
                "ok": False,
                ...
                "version_url": version_url,
                "resolved_url": "",
                "message": f"Cannot reach CDP endpoint: {exc}",
            }
...
def _get_cdp_override() -> str:
    ...
    return _resolve_cdp_override(
        get_session_env("BROWSER_CDP_URL", os.environ.get("BROWSER_CDP_URL", ""))
    )
