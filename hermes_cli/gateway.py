...
def generate_launchd_plist() -> str:
    launchd_support_dir = Path.home() / "Library" / "Application Support" / "Hermes"
    launchd_support_dir.mkdir(parents=True, exist_ok=True)
    launchd_wrapper = launchd_support_dir / "run-hermes-codex-launchd.sh"
    launchd_wrapper.write_text(
        "#!/usr/bin/env bash\n"
        "set -euo pipefail\n"
        f"export HERMES_REPO_ROOT={shlex.quote(str(PROJECT_ROOT))}\n"
        "cd \"$HERMES_REPO_ROOT\"\n"
        "exec /bin/bash \"$HERMES_REPO_ROOT/scripts/run-hermes-codex.sh\" \"$@\"\n",
        encoding="utf-8",
    )
    try:
    ...
    # Route through the Codex bootstrap wrapper so every service start syncs
    # Hermes model/runtime settings from ~/.codex/config.toml first.
    prog_args = [
